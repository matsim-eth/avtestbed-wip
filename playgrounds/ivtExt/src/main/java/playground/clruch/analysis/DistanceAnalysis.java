...
import playground.clruch.math.AnalysisUtils;
...
public class DistanceAnalysis {
	...
	public void analzye() throws Exception {
		...
		totalDistancesPerVehicle = Tensor.of(list.stream().map(vs -> Total.of(vs.distanceTotal)))
				.multiply(RealScalar.of(0.001));
		distancesWCPerVehicle = Tensor.of(list.stream().map(vs -> Total.of(vs.distanceWithCustomer)))
				.multiply(RealScalar.of(0.001));

		AnalyzeAll.totalDistanceBinSize = AnalysisUtils.adaptBinSize(totalDistancesPerVehicle,
				AnalyzeAll.totalDistanceBinSize, RealScalar.of(5.0));
		tdBinCounter = AnalysisUtils.binCount(totalDistancesPerVehicle, AnalyzeAll.totalDistanceBinSize);
		AnalyzeAll.distanceWCBinSize = AnalysisUtils.adaptBinSize(distancesWCPerVehicle, AnalyzeAll.distanceWCBinSize,
				RealScalar.of(5.0));
		...
		Tensor table3 = list.stream().map(vs -> vs.distancePickup).reduce(Tensor::add).get();
		Tensor table4 = list.stream().map(vs -> vs.distanceRebalance).reduce(Tensor::add).get();
		Tensor table5 = table1.map(InvertUnlessZero.FUNCTION).pmul(table2);
